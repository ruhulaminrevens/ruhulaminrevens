import os, queue, subprocess, threading, time, webbrowser, tkinter as tk
...
C={"bg":"#0b1220","panel":"#111b2e","panel2":"#16233b","text":"#edf4ff","muted":"#9fb0c7","accent":"#4f8cff","pass":"#26c281","warn":"#f5b942","fail":"#ff5d73","info":"#60a5fa","manual":"#a78bfa","unknown":"#f5b942","border":"#253651"}
T={
...
class LaptopCheckApp(tk.Tk):
    def __init__(self):
        super().__init__(); self.lang='en'; self.busy=False; self.events=queue.Queue(); self.rows=[]; self.summary={}; self.scanned=False; self.labels={}; self._window(); self._style(); self._ui(); self._seed(); self.refresh(); self.after(100,self._poll_scan)
    ...
    def _auto_tab(self):
        ...
        sc=ttk.Scrollbar(f,orient='vertical',command=self.tree.yview); self.tree.configure(yscrollcommand=sc.set); hsc=ttk.Scrollbar(f,orient='horizontal',command=self.tree.xview); self.tree.configure(xscrollcommand=hsc.set); self.tree.grid(row=0,column=0,sticky='nsew'); sc.grid(row=0,column=1,sticky='ns'); hsc.grid(row=1,column=0,sticky='ew'); f.grid_rowconfigure(0,weight=1); f.grid_columnconfigure(0,weight=1); [self.tree.tag_configure(k,foreground=C[k.lower()]) for k in ['PASS','WARN','FAIL','INFO','MANUAL','UNKNOWN']]
    ...
    def reset(self):
        if self.busy: return
        if not messagebox.askyesno('New inspection','Clear current results and start a new laptop inspection? Export first if needed.'): return
        self.rows=[]; self.summary={}; self.scanned=False
        self._seed(); self.status.config(text=self.x('not'),fg=C['muted']); self.refresh()

    def scan(self):
        if self.busy: return
        self.busy=True; self.prog.start(10)
        self.status.config(text=self.x('scanning'),fg=C['accent'])
        for button in (self.scanb,self.exp,self.resetb): button.config(state='disabled')
        def work():
            try:
                a,s=auto_scan(); self.events.put(('done',a,s))
    ...
    def _poll_scan(self):
        try:
            event,a,s=self.events.get_nowait()
        except queue.Empty:
            pass
        else:
            self.busy=False; self.prog.stop()
            for button in (self.scanb,self.exp,self.resetb): button.config(state='normal')
            if event=='done': self._done(a,s)
            else:
                self.status.config(text='Scan failed — retry available',fg=C['fail'])
                messagebox.showerror('Scan failed',a)
        self.after(100,self._poll_scan)

    def _done(self,a,s):
        keep=[dict(r) for r in self.rows if any(r['item']==x[1] for x in MANUAL)]
        old_serial=self.summary.get('serial'); new_serial=s.get('serial')
        if old_serial and new_serial and old_serial!=new_serial: keep=[]
        self.rows=a; self.summary=s; self.scanned=True
        self.rows.extend(keep); self._seed()
        self.status.config(text=self.x('complete'),fg=C['pass']); self.refresh()
    ...
    def keyboard(self):
        ...
        seen=set(); counter=tk.Label(w,text='Unique keys: 0',bg=C['bg'],fg=C['accent']); counter.pack()
        def record(event):
            seen.add(event.keysym)
            counter.config(text=f"Unique keys: {len(seen)} | Last: {event.keysym}")
        w.bind('<KeyPress>',record,add='+')
    def touch(self):
    ...
    def export(self):
        ...
        if p:
            try:
                export_report(self.rows,self.summary,p)
            except Exception as error:
                messagebox.showerror('Export failed',f'Could not save all report files. Choose a writable folder.\n{error}')
                return
            messagebox.showinfo('Laptop Check Pro',f'HTML + JSON + CSV reports saved.\n{p}')
